#!/bin/bash
USAGE="Usage: $0 -f FILE -d DATABASE -S SERVER [-T or -U USERNAME -P PASSWORD]  ( -d and -f are required; the others have defaults. -T is Integrated auth (Windows-only)"
...
FILTERS=""
OPTIONS="/Properties:DropObjectsNotInSource=True /Properties:IgnoreAuthorizer=True"
...
while [ $# -gt 0 ]; do
	case "$1" in
		...
		-P)
			...
			shift
			;;
		-a) #Deploy All object types, not just tables. (default)
			FILTERS=""
			#no arg, no shift
			;;
		--only-tables)
		...
		--filters)
			...
			;;
		--force)
			OPTIONS="$OPTIONS /Properties:BlockOnPossibleDataLoss=False"
			#no arg, no shift
			;;
		--options)
			OPTIONS="$2"
...
if [ ! -z "$FILTERS" ]; then
	FILTERS="/Properties:AdditionalDeploymentContributors=AgileSqlClub.DeploymentFilterContributor /Properties:AdditionalDeploymentContributorArguments=$FILTERS"
fi

echo Deploying schema from "$FILE" to "$DATABASE" on "$SERVER" using $(dirname "$0")/Database.publish.xml with options \""$OPTIONS"\"" and filters \""$FILTERS"\""

sqlpackage \
	$ACTION /DeployReportPath:"$FILE-deployreport.xml" /DeployScriptPath:"$FILE-script.sql"  \
	/SourceFile:"$FILE" /Profile:"$(dirname "$0")/Database.publish.xml" \
	/TargetConnectionString:"Data Source=$SERVER;$AUTH;Initial Catalog=$DATABASE;" $OPTIONS $FILTERS
